document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderDebug(result) {
        let html = `<ul class="debug-list">`;
        html += `<li><strong>Classification:</strong> <span class="tag ${result.type}">${result.type.toUpperCase()}</span></li>`;
        html += `<li><strong>Year Extraction:</strong> ${result.year}</li>`;
        html += `<li><strong>Display Title:</strong> "${result.display_title}"</li>`;
        html += `<li><strong>Machine Title:</strong> "${result.machine_title}"</li>`;
        
        if (result.season) {
            html += `<li><strong>Season:</strong> ${Array.isArray(result.season) ? result.season.join(', ') : result.season}</li>`;
        }
        
        if (result.episode) {
            html += `<li><strong>Episode:</strong> ${result.episode}</li>`;
        }

        if (result.part || result.volume) {
            html += `<li><strong>Part/Vol:</strong> Part: ${result.part}, Vol: ${result.volume}</li>`;
        }

        if (result.languages.length) {
            html += `<li><strong>Languages:</strong> ${result.languages.join(', ')}</li>`;
        }

        if (result.qualities.length) {
            html += `<li><strong>Qualities:</strong> ${result.qualities.join(', ')}</li>`;
        }
        
        if (result.platform !== 'Unknown') {
            html += `<li><strong>Platform:</strong> ${result.platform}</li>`;
        }

        if (result.update_info) {
             const updates = [];
             if (result.update_info.episode_added) updates.push(`Ep ${result.update_info.episode_added} Added`);
             if (result.update_info.volume_added) updates.push(`Vol ${result.update_info.volume_added} Added`);
             if (result.update_info.part_added) updates.push(`Part ${result.update_info.part_added === true ? 'Added' : result.update_info.part_added + ' Added'}`);
             if (result.update_info.complete) updates.push(`Complete`);
             
             if (updates.length) {
                 html += `<li><strong>Update Info:</strong> ${updates.join(', ')}</li>`;
             }
        }

        html += `<li><strong>Confidence Score:</strong> ${(result.confidence * 100).toFixed(0)}%</li>`;
        html += `</ul>`;
    ...
    runTestsBtn.addEventListener('click', () => {
        ...
        testCases.forEach((title, index) => {
            ...
            row.innerHTML = `
                ...
                <td class="code-font">${result.canonical_key}</td>
                <td><span class="tag ${result.type}">${result.type}</span></td>
                <td>${result.year}</td>
            `;
